...
open class ForegroundService : Service(),TextToSpeech.OnInitListener {
    companion object {
        ...
        private var screen_ratiio by Delegates.notNull<Float>()
        private var dpi by Delegates.notNull<Int>()
        private lateinit var sharedPreferencesHelper: SharedPreferencesHelper
    }

    private var phonpe_current_state : Int = 0
    private var prev_phonpe_current_state : Int = 0
    private var negativeIntentCounter by Delegates.notNull<Int>()
    private lateinit var networkHelper: NetworkHelper
    private lateinit var auth: FirebaseAuth
    private var isBluetoothReceiverRegistered = false
    private var in_callFun = false
    ...
    private var mor_option_activated_for_payment = false
    private var amt : Double = 0.0
    private var recent_amt : Double = 0.0
    private var recent_contact_list_sort_by_voice : MutableList<Contact> = mutableListOf()
    private var recent_call_contact_list_sort_by_voice : MutableList<Contact> = mutableListOf()
    private var recent_payment_number : String = ""
    private var recent_call_number : String = ""
    private var recent_call_name : String = ""
    ...
    **/
    ...
    private val broadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            if(intent?.action == "com.example.yourapp.BROADCAST_ACTION_upi_contacts_updation"){
                val message = intent.getStringExtra("message")
                upi_contacts = sharedPreferencesHelper.getContacts("upi_contacts").toMutableList()
                contactList_transaction = (contactList_only + upi_contacts).distinct()
            }
    ...
    val callbackInterface = object : NumberCallback {
        override fun onRequeststart(action: String) {
            ...
            Log.d(TAG, "aws action means forground - ${action}")
...
// Start the foreground service with the non-dismissible notification
        ...
        **/
        ...
        if (data != null) {
            startProjection(resultCode, data)// for mediaprojection / screenshot purpose
        }*/
    ...
    override fun onInit(status: Int) {
        if (status == TextToSpeech.SUCCESS) {
            ...
            Log.d("trackk","Total nos of voices" + voices.size.toString())
            // 33 - good,
...
// Get the current user's UID
        ...
        upi_contacts =
            sharedPreferencesHelper.getContacts("upi_contacts").toMutableList() // for normal start need to check
        ...
        contactList_transaction = (contactList_only + upi_contacts).distinct()
        Log.d(TAG + " upi_contacts",upi_contacts.toString())
        ...
        negativeIntentCounter = sharedPreferencesHelper.getInt("negativeIntentCounter")

        screenSize = getScreenSize(this)
        val get_without_nav_stat_screenSize = get_without_nav_stat_screenSize(this)
        screen_ratiio = (screenSize.second.toFloat()/get_without_nav_stat_screenSize.second)
        Log.d(TAG, "screen_ratiio : $screen_ratiio")
        Log.d("trackk"," screensize = ${screenSize.first} + ${screenSize.second}")
        dpi = getScreenDPI(this)
        Log.d(TAG, "Screen DPI: $dpi")
    ...
    fun createVirtualDisplay() {
        ...
        //mWidth = getScreenWidth(ForegroundService.mainActivity)
        mHeight = Resources.getSystem().displayMetrics.heightPixels
        Log.d("trackk", "mwidth - $mWidth, mHeight - $mHeight")

        // start capture reader
        mImageReader = ImageReader.newInstance(screenSize.first, screenSize.second, PixelFormat.RGBA_8888, 2)
    ...
    fun getScreenSize(context: Context): Pair<Int, Int> {
        ...
        // This method includes system decorations (e.g., navigation and status bars)
    ...
    fun get_without_nav_stat_screenSize(context: Context): Pair<Int, Int> {
        val windowManager = context.getSystemService(Context.WINDOW_SERVICE) as WindowManager
    ...
    private fun phonepe_calibration_home_home() : Boolean{
        ...
        getStringFromSharedPreferences(applicationContext,"Jiosaavn")?.let {
            ...
            if (arrayValue != null) {
                ...
                //var index = indexOfsidebyside3String_contain(desMap,"to","mobile","number")
                var index = lastIndexOfSideBySideString(desMap,"to","mobile")
                Log.d(TAG,"later download index value - ${index.toString()}")
                if(index == -1){
                    return_state = false
                }
    ...
    private fun lastIndexOfSideBySideString(arraylist: ArrayList<String>, string1: String, string2: String): Int {
        var lastIndex = -1

        for (i in 0 until arraylist.size - 1) {
            if (arraylist[i] == string1 && arraylist[i + 1] == string2) {
                lastIndex = i + 1
            }
        }
            ...
            if (detected) {
                ...
                if (phonpe_current_state > 0) {
                    ...
                    if (detected) {
                                    ...
                                    if (phonepe_calibration_toMobile()){
                                        ...
                                        if (uploadFile_new("Jiosaavn")){
                                            if(phonepe_calibration_toMobile_search()){
                                                ...
                                                SharedData.copy_text = "9733774436@ybl"
                                                SharedData.copy_text_stat = true
                                                delay(500)
                                                sendBroadcastMessage_for_back_button()
                                                delay(2000)
                        ...
                        textToSpeech.speak("oi oi Some intentionally touch done on screen or please check your network or please turn on accessibility service and then off and again on and check",TextToSpeech.QUEUE_ADD,null,null)
                    }
                }
            } else {
    ...
    private suspend fun result(data : ResultData) {
        ...
        networkHelper.sendRequest(action) { response ->
            if (response != null) {
                ...
                // Check if the intent is "make_call"
                if (intent == "affirmative" && in_callFun) {
                    latch = CountDownLatch(1)
                    speakText("ok sir, calling", utteranceId)
                    try {
                        latch.await()
                    } catch (e: InterruptedException) {
                        e.printStackTrace()
                    }
                    makeCall(recent_call_number)
                    in_callFun = false
                }
                if (intent == "make_call" && in_callFun) {
                    val entities = jsonObject.getJSONObject("entities")

                    // Extract the "contact_name" entity if it exists
                    if (!entities.has("contact_name")) {
                        latch = CountDownLatch(1)
                        speakText("ok sir, calling", utteranceId)
                        try {
                            latch.await()
                ...
                if ((intent == "negative" || intent == "angry") && in_callFun) {
                    //in_callFun = false
                    negativeIntentCounter++ // Increment the counter
                    sharedPreferencesHelper.saveInt("negativeIntentCounter",negativeIntentCounter)

                    if (negativeIntentCounter % 3 == 0) {
                        // Call your function every 3rd cycle
                        speakText("sir, if you need more options on this contact name, please feel free to ask", utteranceId)
                    }
                }
                if(intent == "more_options" && in_callFun) {
                    mor_option_activated_for_payment = false
                    if(recent_call_contact_list_sort_by_voice.size > 1){
                        removeContactByNameAndNumber(recent_call_name,recent_call_number)
                        recent_call_number = recent_call_contact_list_sort_by_voice.first().number
                        recent_call_name = recent_call_contact_list_sort_by_voice.first().name
                        speakText("do you want to call ${recent_call_name}", utteranceId)

                    } else {
                        speakText("Sir, no other matched contact found ", utteranceId)
                    }
                }
                if (intent == "make_call") {
                    ...
                    // Extract the "contact_name" entity if it exists
                    if (entities.has("contact_name")) {
                        val contactName = entities.getString("contact_name")
                        //val con = findBestContact(contactName)
                        if (contactList_only.isNotEmpty()) {
                            findBestMatch(contactName,contactList_only)?.let { contac ->
                                recent_call_contact_list_sort_by_voice = contac.toMutableList()
                                // Remove the last element
                                recent_call_contact_list_sort_by_voice =
                                    recent_call_contact_list_sort_by_voice.dropLast(1).toMutableList()
                                // Sort the list by the length of the 'name' string
                                recent_call_contact_list_sort_by_voice =
                                    recent_call_contact_list_sort_by_voice.sortedBy { it.name.length }
                                        .toMutableList()
                                recent_call_number = recent_call_contact_list_sort_by_voice.first().number
                                recent_call_name = recent_call_contact_list_sort_by_voice.first().name
                                latch = CountDownLatch(1)
                            ...
                            speakText("Sir , your contact list is empty", utteranceId)
                        }
                        /*if (con != null) {
                            speakText("do you want to call ${con.name}", utteranceId)
                        }*/
                        println("Intent is 'make_call', Contact Name: $contactName")
                    } else {
                        if(!in_callFun){
                            println("Intent is 'make_call', but no contact name found")
                            speakText("Sir, contact name not clear,  who should I call?", utteranceId)
                        }
                    }
                    wait_ver_new = 1
                }
                if (intent == "greeting") {
                    speakText("Hello sir, what can i do for you?", utteranceId)
                    wait_ver_new = 1
                }
                if (intent == "angry") {
                    speakText("I understand you're upset, can you please try again?", utteranceId)
                    wait_ver_new = 1
                }
                if(sharedPreferencesHelper.getBoolean("isPhonepeFeaturesOn")){
                    if(intent == "affirmative"){
                        ...
                        if(mor_option_activated_for_payment) {
                            Log.d(TAG,"affi2")
                            ...
                            open_specialOpen()
                            SharedData.back_button_state = true
                            SharedData.back_button_state = true
                            Thread.sleep(1000)
                            simulate_HomeButton()
                            Log.d(TAG,"affi3")
                            ...
                            GlobalScope.launch(Dispatchers.Main) {
                                ...
                                if (detected) {
                                    Log.d(TAG,"affi5")
                                    //backTo_number_method() // special screen set
                                    backTo_number_method_new()
                                    delay(500)
                                    /*if(contact_list_sort_by_voice.size>2) {
                                        SharedData.copy_text = recipient_name_final
                                        SharedData.copy_text_stat = true
                                        delay(3000)
                                        SharedData.get_recentList = true
                                    }*/
                                    SharedData.copy_text = recent_payment_number
                                    SharedData.copy_text_stat = true
                                    delay(1500)
                                    SharedData.touchByText_text = recent_payment_number
                                    SharedData.touchByText_contains = true
                                    // oio oi oi oi
                                    val startTime = System.currentTimeMillis()
                                    var task_executed = false
                                    while (System.currentTimeMillis() - startTime < 10000) {
                                        phonpe_current_state = MyAccessibilityService.phonepe_activity_state
                                        if(phonpe_current_state == 3) {
                                            delay(500)
                                            SharedData.copy_text = recent_amt.toString()
                                            SharedData.copy_text_stat = true
                                            task_executed = true
                                            delay(2000)
                                            SharedData.touchByText_text = "pay"
                                            SharedData.touchByText_noParent = true
                                            break
                                        }
                                    ...
                                    if(task_executed){
                                        ...
                                        while (System.currentTimeMillis() - startTime < 10000) {
                                            ...
                                            if(phonpe_current_state == 4) {
                                                delay(500)
                                                SharedData.pin = "983230"
                                                SharedData.enter_pin = true
                                                task_executed = true
                                                break
                                            }
                                            withContext(Dispatchers.IO) {
                    ...
                    if(intent == "more_options"){
                        ...
                        GlobalScope.launch(Dispatchers.Main) {
                            ...
                            if (detected) {
                                ...
                                //SharedData.copy_text = recent_wit_name
                                //SharedData.copy_text_stat = true
                                phonepe_name_identification_for_moreOption_new(recent_contact_list_sort_by_voice,recent_processedWit_name)
                            } else {
                                speakText("Some intentionally touch done on screen or please check your network or please turn on accessibility service and then off and again on and check",utteranceId)
                            }
                            wait_ver_new = 1
                            ...
                            GlobalScope.launch(Dispatchers.Main) {
                                ...
                                if(recent_contact_list_sort_by_voice.size > 2){
                                    ...
                                    recent_contact_list_sort_by_voice =
                                        recent_contact_list_sort_by_voice.filter { contact -> contact.number != recent_payment_number }
                                            .toMutableList()
                                    phonepe_name_identification_for_moreOption_new(recent_contact_list_sort_by_voice,recent_processedWit_name)
                                }
                    ...
                    if(intent == "money_transfer"){
                        ...
                        // Extract the "contact_name" entity if it exists
                        mor_option_activated_for_payment = true
                        amt = 0.0
                        //wait_vr = 0
                        // Retrieve money recipient
                        var recipient_name = ""
                        if (entities.has("contact_name")) {
                            recent_wit_name = entities.getString("contact_name")
                            recipient_name = recent_wit_name
                        }
                        var recipient_name_final= ""
                        var contact_list_sort_by_voice : List<Contact> = listOf()
                        if (entities.has("amount")) {
                            amt = entities.getString("amount").toDouble()
                        }
                        ...
                        recent_contact_list_sort_by_voice = contact_list_sort_by_voice.toMutableList()
                        ...
                        /*if(contact_list_sort_by_voice.size == 2){
                            ...
                            recent_contact_list_sort_by_voice.removeAt(0)
                            sendBroadcastMessage2("hi")
                            recent_amt = amt
                            recent_payment_number = contact_list_sort_by_voice.first().number
                            recent_payment_name = contact_list_sort_by_voice.first().name
                        ...
                        GlobalScope.launch(Dispatchers.Main) {
                            ...
                            if (detected) {
                                ...
                                backTo_number_method_new()
                                delay(500)
                                Log.d(TAG,"recipient_name_final - $recipient_name_final")
                                /*if(contact_list_sort_by_voice.size>2) {
                                    SharedData.copy_text = recipient_name_final
                                    SharedData.copy_text_stat = true
                                    delay(3000)
                                ...
                                phonepe_name_identification_new(contact_list_sort_by_voice,recipient_name_final, utteranceId)
                            } else {
                                speakText("Some intentionally touch done on screen or please check your network or please turn on accessibility service and then off and again on and check",utteranceId)
                            }
                            wait_ver_new = 1
        ...
        witAiHelper.sendWitRequest(sessionId, contextMapp, "", token) { witResponse ->
            witResponse?.let {
                println("Final response: ${it.response?.text}")
                Log.d(TAG,"Final response: ${it.action.toString()}")
                ...
                if(it.action == "input" || it.action == "more_action" || it.action == "send_money"){
                    Log.d(TAG,"wait_vr = 0 of wit.ai")
                }else {
                    Log.d(TAG,"wait_vr = 1 of wit.ai")
                    wait_vr = 1
                }
        ...
        if(action.contains("send data")){
            //fetchDataFromFirestore("phonepay")
            //uploadFile_new("Jiosaavn")
//            SharedData.copy_text = "983230"
//            SharedData.copy_text_stat = true
            SharedData.touchByText_text = "pay"
            SharedData.touchByText = true
            /*SharedData.touchByText_text = "search any contact"
            SharedData.touchByText = true
            SharedData.touchByText_text = "search any mobile"
            SharedData.touchByText = true
            SharedData.touchByText_text = "start a new payment"
            SharedData.touchByText = true*/
        }
        ...
        if(containsAllWords("bluetooth mic on",action.lowercase())){
            if(bluetooth_connection_state){
                audioManager.isBluetoothScoOn = true
                audioManager.startBluetoothSco()
                delay(500)
                Log.d(TAG,"Your Bluetooth device mic is connected")
                //speakText("Your Bluetooth device mic is connected", utteranceId)
            }
        }
        if(containsAllWords("bluetooth mic off",action.lowercase())){
            if (bluetooth_connection_state){
                audioManager.isBluetoothScoOn = false
                audioManager.stopBluetoothSco()
                delay(500)
                Log.d(TAG,"Your Bluetooth device mic is DISconnected")
                //speakText("Your Bluetooth device mic is disconnected", utteranceId)
            }
        }
        if(action.contains("bluetooth//m")){
            GlobalScope.launch {
                delay(400) // Replace with a coroutine-friendly delay
        ...
        while (!(wait_ver_new == 1 && wait_vr == 1)) {
            delay(500) // Non-blocking delay, coroutine-friendly
            Log.d("new", "${wait_ver_new},${wait_vr}")
        }
    }

    private suspend fun phonepe_name_identification_for_moreOption_new(contact_list_sort_by_voice : List<Contact>,recipient_name_final : String){
        var brake_con = false
        var name_name = ""
        val list_size = recentList_in_input.size
        val sizz = recent_contact_list_sort_by_voice.size

        if(list_size != 0){
            recent_payment_name = recentList_in_input.first()
            recentList_in_input.removeAt(0)
            latch = CountDownLatch(1)
            ...
            if(sizz == 0){
                ...
                findBestMatch_by_leven_only(recipient_name_final,recent_contact_list_sort_by_voice)?.let {
                    recent_payment_name = it.first().name
                    recent_payment_number = it.first().number
                    deleteContact(recent_contact_list_sort_by_voice,it.first().number)
                    latch = CountDownLatch(1)
                    speakText("Sir, do you want to send ${recent_amt.toInt()} rupees to ${recent_payment_name}",utteranceId)
                    // Wait for the speech to finish
                    try {
        ...
        /*for (i in 0 until sizz){
            ...
            findBestMatch_by_leven_only(name_name,recent_contact_list_sort_by_voice)?.let {
                ...
                if (bol_result){
                    recent_contact_list_sort_by_voice =
                        recent_contact_list_sort_by_voice.filter { coontact -> coontact.number != it[0].number }
                            .toMutableList()
                    //sendBroadcastMessage_for_back_button()
                    //delay(2000)
                    //execute("search_box_delete_text","phonepe_array")
    ...
    private suspend fun phonepe_name_identification_new(contact_list_sort_by_voice : List<Contact>,recipient_name_final : String, utteranceId : String){
        if(recent_contact_list_sort_by_voice.size>1){
            recent_amt=amt
            SharedData.copy_text = recipient_name_final
            SharedData.copy_text_stat = true
            delay(2000)
            SharedData.get_recentList = true
            var recentList: MutableList<String> = mutableListOf()
            val startTime = System.currentTimeMillis()
            while (System.currentTimeMillis() - startTime < 5000) {
                if (!SharedData.get_recentList) {
                    recentList = SharedData.get_recentList_list.toMutableList()
                    break
                }
                withContext(Dispatchers.IO) {
                    Thread.sleep(500)
            ...
            recent_contact_list_sort_by_voice.removeLast()
            // Debugging: Log the results
            if (recent_contact_list_sort_by_voice.isNotEmpty()) {
                ...
                if(recentList.isNotEmpty()){
                    ...
                    //recent_payment_number = recentList_in_input[0].number
                    for(i in 0 until recentList.size) {
                        val payment_name = recentList_in_input[i]
                        findBestMatch_by_leven_only(payment_name,recent_contact_list_sort_by_voice)?.let {
                            var index = 0
                            for (char in it.first().name) {
                                if (index < payment_name.length && char == payment_name[index]) {
                                    index++ // Move to the next character in the candidate
                                }
                            }
                    ...
                    recent_payment_name = recentList_in_input.first()
                    recent_payment_number = recent_payment_name
                    recentList_in_input.removeAt(0)
                    latch = CountDownLatch(1)
                    //val utteranceId = UUID.randomUUID().toString()
                    speakText("Sir, do you want to send ${amt.toInt()} rupees to $recent_payment_name",utteranceId)
                    ...
                    phonepe_name_identification_for_moreOption_new(contact_list_sort_by_voice,recipient_name_final)
                }
            } else {
    ...
    private suspend fun backTo_number_method_new(){
        delay(1000)
        if(SharedData.accessibility_changed){
            SharedData.accessibility_changed = false
            prev_phonpe_current_state = phonpe_current_state
            phonpe_current_state = MyAccessibilityService.phonepe_activity_state
            Log.d(TAG,"now current state = $phonpe_current_state")
            if (phonpe_current_state == 2){
                Log.d(TAG, "entering state 2 and prev - ${prev_phonpe_current_state  } ")
                if(prev_phonpe_current_state == 1){
                    SharedData.touchByText_nextView = "to any upi app"
                    SharedData.touchBynextView = true
                } else {
                    SharedData.back_button_state = true
                    Log.d(TAG,"state - " + SharedData.input_accessi_activated.toString())
                    if(SharedData.input_accessi_activated){
                ...
                SharedData.touchByText_text = "home"
                SharedData.touchByText = true
                delay(100)
                SharedData.touchByText_text = "to mobile number"
                SharedData.touchByText = true
                backTo_number_method_new()
    ...
    private suspend fun backTo_number_method(){
        phonpe_current_state = MyAccessibilityService.phonepe_activity_state
        Log.d(TAG,"phonpe_current_state + $phonpe_current_state")
        if (phonpe_current_state == 2){
            delay(2000)
            takescreenshot()
            val bol_result =
    ...
    fun findBestContact(query: String): Contact? {
        val levenshtein = Levenshtein()
        var bestContact: Contact? = null
        var highestScore = Double.NEGATIVE_INFINITY

        for (contact in contactList_only) {
            val contactName = contact.name.lowercase()
            val queryLower = query.lowercase()

            // Skip if the contact has no valid number
            if (contact.number.isNullOrEmpty()) continue

            // Exact Match (highest score)
            if (contactName == queryLower) {
                return contact // Return immediately if an exact match is found
            }

            // Partial Match (check if the query is contained in the contact name)
            if (contactName.contains(queryLower)) {
                val score = 1.0 // Assign a high score for partial matches
            ...
            // Fuzzy Match (calculate similarity score)
            val similarityScore = 1 - levenshtein.distance(contactName, queryLower) / maxOf(contactName.length, queryLower.length).toDouble()
            ...
            val tokenMatchScore = weightedTokenMatchScore(contactName, queryLower)
            if (tokenMatchScore > highestScore) {
                highestScore = tokenMatchScore
                bestContact = contact
            }

            // Bigram Matching
            val bigramMatchScore = bigramMatchScore(contactName, queryLower).toDouble()
            if (bigramMatchScore > highestScore) {
                highestScore = bigramMatchScore
                bestContact = contact
            }
        }
    ...
    fun weightedTokenMatchScore(contactName: String, query: String): Double {
        val contactTokens = contactName.lowercase().split(" ")
        val queryTokens = query.lowercase().split(" ")

        var score = 0.0
        val weights = listOf(1.0, 0.5, 0.3) // Weight for first, second, third tokens, etc.

        for ((i, contactToken) in contactTokens.withIndex()) {
            val weight = weights.getOrElse(i) { 0.1 } // Use lower weight for extra tokens
            for (queryToken in queryTokens) {
                if (contactToken.startsWith(queryToken)) {
                    score += weight
                }
            }
        }
    ...
    fun generateBigrams(input: String): List<String> {
        val bigrams = mutableListOf<String>()
        val cleanedInput = input.lowercase().replace(" ", "")
        for (i in 0 until cleanedInput.length - 1) {
            bigrams.add(cleanedInput.substring(i, i + 2))
        }
        return bigrams
    }

    fun bigramMatchScore(contactName: String, query: String): Int {
        val contactBigrams = generateBigrams(contactName)
        val queryBigrams = generateBigrams(query)

        return contactBigrams.intersect(queryBigrams).size
    }

    fun makeCall(phoneNumber: String) {
        val callIntent = Intent(Intent.ACTION_CALL) // Use ACTION_CALL for direct calls
        callIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) // Add this line to fix the error
        ...
        if (checkSelfPermission(android.Manifest.permission.CALL_PHONE) == PackageManager.PERMISSION_GRANTED) {
            ...
            Log.d(TAG, "Permission not granted to make calls")
        }
    }
    fun containsAllWords(input: String, target: String): Boolean {
        // Split the strings into words and convert them to sets for easier comparison
        val inputWords = input.split("\\s+".toRegex()).map { it.trim() }.filter { it.isNotEmpty() }
        val targetWords = target.split("\\s+".toRegex()).map { it.trim() }.filter { it.isNotEmpty() }.toSet()

        // Check if all input words are in the target words
        return inputWords.all { it in targetWords }
    }

    fun removeContactByNameAndNumber(matchName: String, matchNumber: String) {
        recent_call_contact_list_sort_by_voice =
            recent_call_contact_list_sort_by_voice.filterNot { contact ->
                contact.name.equals(matchName, ignoreCase = true) && contact.number == matchNumber
            }.toMutableList()
    }
    fun deleteContact(
        contactList: MutableList<Contact>,
        phoneNumberToDelete: String
    ): Boolean {
        val iterator = contactList.iterator()
        while (iterator.hasNext()) {
            val contact = iterator.next()
            if (contact.number == phoneNumberToDelete) {
                iterator.remove() // Safely remove the contact
                return true // Indicate successful deletion
            }
        }
        return false // Indicate no matching contact was found
    }
